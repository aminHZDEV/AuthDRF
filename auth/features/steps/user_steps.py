...
def step_impl(context):
    data = {heading: row[heading] for heading in context.table.headings for row in context.table}
    response = context.client.post(reverse("user-register"), data, format="json")
    context.response = response

@then(u'I should receive a confirmation with status 201')
def step_impl(context):
    assert context.response.status_code == status.HTTP_201_CREATED

@then(u'the response should contain the following details after register')
def step_impl(context):
    expected_data = {heading: row[heading] for heading in context.table.headings for row in context.table}
...
@then(u'I should receive a token with status 200')
def step_impl(context):
    assert context.response.status_code == status.HTTP_200_OK

@then(u'the token response should contain the following details')
def step_impl(context):
    expected_data = {heading: row[heading] for heading in context.table.headings for row in context.table}
    response_data = context.response.json()
    for key, value in expected_data.items():
...
@then(u'I should store the access token for subsequent requests')
def step_impl(context):
...
def step_impl(context):
    ...
    if 'auth_token' in context and context.auth_token is not None:
        ...
        raise AssertionError("Authentication token not found. Please obtain an access token first.")
...
@then(u'I should receive a list of users with status 200')
def step_impl(context):
    assert context.response.status_code == status.HTTP_200_OK
...
def step_impl(context):
    ...
    for expected_user in expected_data:
        ...
        assert username in response_user_dict, f"User {username} not found in response data."
        assert response_user_dict[username]['phone_number'] == expected_user['phone_number'], \
        ...
        assert response_user_dict[username]['email'] == expected_user['email'], \
